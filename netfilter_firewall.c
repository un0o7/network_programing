 ...
 void getAddress(long saddr, char *str)
{
               ...
               struct nfq_data *nfa, void *data)
 {
    ...
    char ipaddr[20];
    char ipsaddr[20];
    getAddress(iphdrp->daddr, ipaddr);
    getAddress(iphdrp->saddr, ipsaddr);
     printf("源 ip %s\n", ipaddr);
        printf("目标ip %s\n", ipsaddr);
     printf("len=%d\n",strlen(ipaddr));
    if (strcmp(ipsaddr,"10.133.61.172")==0 && strcmp(ipaddr,"10.133.60.185")==0){
 ...
 int main(int argc, char **argv)
 {
         ...
         int fd;
         int rv;
         uint32_t queue = 0;//队列编号，65535中的一个
